...
if(isset($_POST['login_btn'])){
    ...
    if($stmt->num_rows() == 1){
        $stmt->bind_result($admin_id, $admin_name, $admin_email, $admin_password_hashed);
        $stmt->fetch();

        // Verify hashed password
        if (md5($password) == $admin_password_hashed) {
            $_SESSION['admin_id'] = $admin_id;
            $_SESSION['admin_name'] = $admin_name;
            $_SESSION['admin_email'] = $admin_email;
            $_SESSION['admin_logged_in'] = true;

            header('location: dashboard.php?login_success=logged in successfully'); // Redirect to dashboard
        } else {
            header('location: alogin.php?error=Incorrect email or password');
        }
    } else {
        header('location: alogin.php?error=Incorrect email or password');
    }
...
<body>
    ...
    <div class="container text-center mt-2 pt-5">
        <h2 class="script-font">Admin Login</h2>
        <hr class="mx-auto">
    </div>
    <div class="mx-auto container">
        <form id="login-form" enctype="multipart/form-data" method="POST" action="alogin.php">
